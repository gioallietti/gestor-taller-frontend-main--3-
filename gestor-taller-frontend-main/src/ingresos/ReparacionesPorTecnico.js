...
const ReparacionesPorTecnico = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const [ingresosRes, usuariosRes] = await Promise.all([
                    fetch(urlBase + '/ingresos/reparados/2/2'),
                    fetch(urlBase + '/usuarios/tecnicos/2'),
                ]);

                if (ingresosRes.ok && usuariosRes.ok ) {
    ...
    }, [fechaDesde, fechaHasta, ingresos]);

    const ingresoPagina = (ingreso) => {
        navigate(`/taller/ingresos/${ingreso.id}`);
    }

    const filtrarPorTecnico = (ingresos, tecnicoId) => {
        if (tecnicoId === 0) {
            return ingresos;
        }
        return ingresos.filter((ingreso) => ingreso.reparadoPor.id === tecnicoId);
    };
                ...
                <Row className="row justify-content-center">
                    ...
                    <Col className='p-1' style={{ Height: '470px', overflowY: 'auto' }}>
                        ...
                        <ul className='list-unstyled '>
                            {filtrarPorTecnico(ingresoFiltrado, tecnicoId).map((ingreso) => (
                                        ...
                                        <Row onClick={() => ingresoPagina(ingreso)} className="d-flex cursor">
                                            ...
                                            <Col xs={12} sm={10} lg={11} className='usuarios-list-item cursor'>
                                                <Row className="d-flex">
                                                    ...
                                                    <Col xs={12} sm={6} lg={3} className="card-text mt-0">{ingreso.reparadoPor.nombre} {ingreso.reparadoPor.apellido}</Col>
...
export default ReparacionesPorTecnico;